...
gcloud run services update realm-rivalry \
  --region us-east5 \
  --clear-env-vars
echo "🚀 Deploying to Cloud Run (minimal version)..."
...
echo "📋 Check logs: gcloud logging read 'resource.type=cloud_run_revision AND resource.labels.service_name=realm-rivalry' --limit=20 --format='table(timestamp,severity,textPayload)'"
