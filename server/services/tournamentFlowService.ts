      ...
      if (completedMatches.length === roundMatches.length && roundMatches.length > 0) {
        ...
        if (nextRoundMatches.length === 0) {
          ...
          if (completedRound === 3) {
            ...
            await this.generateNextRoundMatches(tournamentId, completedRound, nextRound);
  ...
  private async generateNextRoundMatches(tournamentId: number, completedRound: number, nextRound: number): Promise<void> {
    ...
    try {
      ...
      if (currentRoundMatches.length === 0) {
        logInfo(`No completed matches found for round ${completedRound}`);
        return;
      }

      // Get winners from current round
      const winners = currentRoundMatches.map((match: any) => {
        const homeWon = (match.homeScore || 0) > (match.awayScore || 0);
        return homeWon ? match.homeTeamId : match.awayTeamId;
      });

      logInfo(`Found ${winners.length} winners from round ${completedRound}: ${winners.join(', ')}`);

      // Generate matches based on round type
      const nextRoundMatches = [];
      
      if (nextRound === 2) {
        // Generate semifinals (2 matches from 4 winners)
        ...
        // Generate finals (1 match from 2 winners)
        if (winners.length === 2) {
          nextRoundMatches.push({
            tournamentId: tournamentId,
      ...
      // Create the matches in database
      if (nextRoundMatches.length > 0) {
        await prisma.game.createMany({
          data: nextRoundMatches
        });
        
        logInfo(`Created ${nextRoundMatches.length} matches for round ${nextRound}`);
      } else {
        logInfo(`No matches to create for round ${nextRound}`);
      }

    } catch (error) {
      console.error(`Error generating matches for round ${nextRound}:`, error);
    }
  }
